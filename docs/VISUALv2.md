...
## A6. Host Control Panel

The host control panel is the presenter's command centre during a live event. Its design must prioritise **clarity under pressure** — a host presenting in front of an audience needs to locate and trigger any action within one glance and one tap, without accidentally triggering a neighbouring action.

### Control Button System

Host slide controls are grouped into **labelled rows** by function. Each row has a small uppercase label above it (Plus Jakarta Sans, 11px, Stone `#A8A29E`, letter-spacing 0.06em). This grouping eliminates the visual noise of a flat row of identically-weighted buttons.

#### Button Variants

| Variant | When to use | Visual treatment |
|---------|-------------|-----------------|
| **Primary** | The single most likely next action (e.g. "Next slide") | Deep Teal fill `#0F766E`, white text, shadow-xs |
| **Default** | Standard actions with no current state | White fill, Pebble border, Slate text, shadow-xs |
| **Active state** | Toggleable actions currently ON (e.g. "Show feedback") | Seafoam fill `#CCFBF1`, Teal border `#0F766E`, Teal text |
| **Danger** | Irreversible or high-stakes actions (e.g. "End event") | Blush fill `#FEF2F2`, `#FECACA` border, Rosewood text |
| **Dashed / Pause** | Presenter pause — a deliberate, non-permanent hold state | White fill, Pebble dashed border, Slate text |
...
Hover behaviour lifts the button by 1px (`transform: translateY(-1px)`) and upgrades the shadow to `--shadow-sm`. Active/pressed state returns it to `translateY(0)`.

#### Grouped Control Rows

The three functional groups are:

1. **Navigation** — "Next slide", "Skip slide". The primary action (Next slide) always sits leftmost and uses the Primary variant. Navigation is always visible regardless of slide type.

2. **Audience controls** — "Lock answers", "Show feedback", "Reveal answer", "Leaderboard". These are toggleable. When a toggle is ON, the button switches to the Active state variant. Copy flips to reflect the new state: "Lock answers" → "Unlock answers", "Reveal answer" → "Hide answer".

3. **Presenter tools** — "Storyteller pause" and any future ambient controls. Uses the Dashed variant to visually signal its optional, meditative nature. When active, the border becomes solid and the button enters the Active state.

#### Icon Guidance

Every control button carries a 14×14px Lucide icon at `stroke-width: 2`, left-aligned before the label. Icons have `opacity: 0.65` by default and `opacity: 1` on hover and in active states. Icon choice must be immediately recognisable without reading the label — this is essential for rapid glancing during a live event.

| Action | Icon |
|--------|------|
| Next slide | `play` (filled triangle) |
| Skip slide | `skip-forward` |
| Lock answers | `lock` |
| Unlock answers | `unlock` |
| Show / Hide feedback | `check` |
| Reveal answer | `info` |
| Leaderboard | `list` |
| Storyteller pause | `pause` |
| End event | `square` (stop) |

#### End Event Button

The "End event" button lives in the **sidebar footer**, not in the control card. This physical separation prevents accidental triggering during normal operation. It uses the Danger variant but with reduced visual weight: no full-width red fill (as seen in pre-redesign), only a Blush background with a soft `#FECACA` border.

---

<br>

## A7. Adaptive Audience Preview Panel

The right-hand preview panel gives the host a live mirror of what their audience sees on their personal devices. Because audiences join from a variety of device types, the preview must be **switchable between device form factors** so the host can verify the experience at a glance.

### Device Switcher

A compact switcher control sits in the preview panel header (right-aligned). It uses a pill-shaped container (`background: Linen`, `border: 1px solid Pebble`, `border-radius: var(--radius-md)`, `padding: 3px`). Each device button inside it is icon-only (Lucide `smartphone` and `tablet`), 13×13px. The active device button has a white background and `--shadow-xs`; inactive buttons are transparent.
...
Switching devices is **instant** — no animation needed, since this is a utilitarian mode switch not a storytelling moment.

### Device Frames

Both frames use a dark Charcoal shell (`#1C1917`) to simulate a neutral device surround. The shell avoids manufacturer-specific chrome (no camera cutouts, branded logos, or coloured bezels) to keep the host's attention on the screen content, not the frame.

| Property | Phone | Tablet |
|----------|-------|--------|
| Frame width | 180px | 260px |
| Shell border-radius | 24px | 20px |
| Screen aspect ratio | 9:16 | 4:5 |
| Shell padding | 12px 8px | 14px 10px |
| Notch width | 40px | 50px |

Both frames include a home indicator bar (32px wide, 4px tall, `#2C2826`, pill-shaped) below the screen — a neutral, cross-platform affordance that signals "this is a device" without brand specificity.

### Screen Content (Lobby / Welcome State)

The audience welcome screen uses the Deep Teal gradient as a full-bleed background (`#0F766E → #0D6359`, 160deg angle). White text on teal exceeds WCAG AA contrast at all sizes used.

| Element | Phone | Tablet |
|---------|-------|--------|
| Logo wordmark | 14px Fraunces | 16px Fraunces |
| Sparkle emoji | 18px | 22px |
| "Welcome in" heading | 13px Plus Jakarta Sans 700 | 16px Plus Jakarta Sans 700 |
| Supporting copy | 10px Inter, 65% white | 11px Inter, 65% white |
| Waiting animation | 3 dots, 5px, bouncing | 3 dots, 5px, bouncing |
| Join code pill | Not shown | Shown — JetBrains Mono, 10px |

The **join code pill** is only surfaced in the tablet view because tablet users typically have the physical space to read and act on an alphanumeric code. Phone users are expected to have already joined via QR or link. The pill uses `rgba(255,255,255,0.15)` fill with a `rgba(255,255,255,0.2)` border and `border-radius: 9999px`.

### Waiting Animation

Three dots arranged horizontally, each 5×5px, Deep Seafoam (`#CCFBF1`), animate with a staggered vertical bounce:
...
.dot-1 { animation: bounce 1.2s ease-in-out infinite; }
.dot-2 { animation: bounce 1.2s ease-in-out infinite 0.2s; }
.dot-3 { animation: bounce 1.2s ease-in-out infinite 0.4s; }
```

This animation is slow and gentle — it communicates "patiently waiting" rather than urgency.

### Future Device States

The preview panel is designed to be extended. Placeholder device states to implement in future iterations:

| State | Trigger | Preview content |
|-------|---------|----------------|
| **Question active** | Host advances to a question slide | Answer choice buttons, question prompt text |
| **Locked** | Host presses "Lock answers" | Greyed-out choices, "Answers locked" banner |
| **Revealed** | Host presses "Reveal answer" | Correct answer highlighted in Forest green |
| **Leaderboard** | Host opens leaderboard | Ranked list with tier badges |
